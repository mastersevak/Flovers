...
class EShoppingCart extends CMap {
	...
	/**
	 * Model where to save shopping cart items
	 * Your model must have the following fields:
	 * id_user		- ID of the owner(user) of the cart
	 * id_product	- ID of the product which has been add to the cart
	 * quantity		- quantity of the product which has been add to the cart
	 * @var CActiveRecord
	 */
	public $model;
	...
	/**
	 * Save items in cart into base
	 */
	public function saveToCart(){
		$model = $this->model;
		
		$positions = Yii::app()->shoppingCart->getPositions();
		$oldPositions = $model::model()->findAll('id_user = :id_user', [':id_user' => user()->id]);

		$oldPositionsIds = CHtml::listData($oldPositions, 'id_product', 'id');

		foreach($positions as $position){
			$cartPosition = array_key_exists($position->id, $oldPositionsIds) ? 
				$model::model()->findByPk($oldPositionsIds[$position->id]) : 
					new $model;

			$cartPosition->id_user = user()->id;
			$cartPosition->id_product = $position->id;
			$cartPosition->quantity = $cartPosition->quantity ? ($cartPosition->quantity + $position->getQuantity()) : $position->getQuantity();
	...
	/**
	 * Get items from base and fill into cart
	 */
	public function getFromDb(){
		$model = $this->model;
		$positions = $model::model()->findAll('id_user = :id_user', [':id_user' => user()->id]);
		...
		foreach($positions as $position){
			...
			if($this->itemAt($key) instanceof IECartPosition) {
				$oldProduct = $this->itemAt($key);
				$oldQuantity = $oldProduct->getQuantity();
	 ...
	 * @param mixed $key
	 */
	public function remove($key, $position = false) {
		parent::remove($key);

		if($position){
			$model = $this->model;
			$pos = $model::model()->find('id_user = :id_user AND id_product = :id_product', [
				':id_user' => user()->id,
	...
	/**
	 * Returns dataProvider with all positions in the cart
