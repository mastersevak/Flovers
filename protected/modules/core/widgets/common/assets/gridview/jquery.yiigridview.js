	...
	methods = {
			...
			return this.each(function () {
				...
				if (settings.ajaxUpdate.length > 0) {
					$(document).on('click.yiiGridView', settings.updateSelector, function () {
						...
						if (settings.enableHistory && window.History.enabled) {
							...
							//добавил я (нужно если захотим менять method при $.ajax)
							var _opt = {url: $(this).attr('href')};
							
							if($grid.data('update-method') != undefined)
								_opt.type = $grid.data('update-method');

							$('#' + id).yiiGridView('update', _opt);
						}
						return false;
					});
				}
				...
				$(document).on('change.yiiGridView keydown.yiiGridView', settings.filterSelector, function (event) {
					...
					if (settings.enableHistory && settings.ajaxUpdate !== false && window.History.enabled) {
						...
						//эту часть добавил я (отрезаю пустые параметры)
						removeEmpty(params);
		...
		update: function (options) {
			...
			return this.each(function () {
				...
				//добавил я (нужно если захотим менять method при $.ajax)
				var _type = $grid.data('update-method') != undefined ? $grid.data('update-method') : 'GET';
