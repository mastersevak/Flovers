...
class EMigrateCommand extends MigrateCommand
{
	...
	public function getModulePaths()
	{
		if ($this->_modulePaths === null) {
			...
			$this->_getModulesPaths($this->_modulePaths);
		}
	...
	private function _getModulesPaths(&$paths, $_modules = false, $prefix = false){
		
		$modules = $_modules ? $_modules  : Yii::app()->modules;
		
		foreach($modules as $module => $config) {
			if (is_array($config)) {
				$alias = (!$prefix ? 'application.modules.' : $prefix.'.') . $module . '.migrations';

				if (isset($config['class'])) {
					Yii::setPathOfAlias($alias, dirname(Yii::getPathOfAlias($alias)) . '/migrations');
				...
				if(isset($config['modules'])){ //для случаев с подмодулями
					foreach(Yii::app()->getModule($module)->modules as $submodule => $subconfig){
						$this->_getModulesPaths($paths, $subconfig, (!$prefix ? 'application.modules.' : $prefix.'.') . $module );
					}
				}

			} else {
				if(!$prefix){
					$paths[$config] =  'application.modules.' . $config . '.migrations';
				}
				else{
					//для случаев с подмодулями
					$parent = substr($prefix, strrpos($prefix, ".")+1);
					$config = strstr($config, '.', true);
					$paths[$parent.".".$config] = $prefix . '.';
					$paths[$parent.".".$config] .= "modules.".$config;
					$paths[$parent.".".$config] .= '.migrations';
				}
			}
		}
	}
