...
class RobokassaPaymentSystem extends BasePaymentSystem
{
	...
	public function processPaymentRequest(StorePaymentMethod $method)
	{
		...
		// Check sum
		if ($out_sum != Yii::app()->currency->convert($order->full_price, Yii::app()->currency->active->id, $method->currency_id))
			return ERROR_SUM;
	...
	public function renderPaymentForm(StorePaymentMethod $method, Order $order)
	{
		...
		// Order sum
		$out_sum = Yii::app()->currency->convert($order->full_price, Yii::app()->currency->active->id, $method->currency_id);
		// currency
		$in_curr = "PCR";
		// Language
		$culture = "ru";
