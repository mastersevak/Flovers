...
/**
 * Я ПОМЕНЯЛ ИСПОЛЬЗОВАНИЕ
 * чтобы он не удалял записи, перед сохранением, в каждом случае
 *
 * $model->manyMany->save(true, null, array('types'));
 * где manyMany название behavior, а types, та MANY_MANY, связь, для которого нужно сохранение
 */
...
class CAdvancedArbehavior extends CActiveRecordBehavior
{
	...
	//custom save
	public function save($runValidation=true, $attributes=null, $relations=array()){
		...
		if($this->owner->save($runValidation, $attributes)){
			...
			$this->writeManyManyTables($relations);
	...
	protected function writeManyManyTables($relations) 
	{
		if($this->trace)
		...
		if(count($_relations) > 0)
			$this->cleanRelation(current($_relations));
		
		foreach($_relations as $key => $relation) 
		{
