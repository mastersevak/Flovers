<?php
/**
 * MultilangualBehavior class file
 *
 * @author guillemc, Frédéric Rocheron<frederic.rocheron@gmail.com>
 */
/**
 * MultilangualBehavior handles active record model translation.
 *
 ...
 *     return array(
 *         'ml' => array(
 *             'class' => 'application.models.behaviors.MultilangualBehavior',
 ...
 * 
 */
class MultilangualBehavior extends CActiveRecordBehavior
{
    /**
    ...
    public function modifySearchCriteria(CDbCriteria $criteria) {
        ...
        $criteria = new SDbCriteria($criteria_array);
	...
	public function afterFind($event) {
        ...
        if ($owner->hasRelated($this->multilangRelation)) {
			...
			if ($row = current($related)) {
				...
				foreach ($this->localizedAttributes as $field){
                    if (property_exists($owner, $field) && (!empty($row[$this->localizedPrefix . $field]) || $this->forceOverwrite)) {
