...
(function($) {
	...
	*/
	$(document).ready(function() {
		$('.inherited-item').rightsTooltip({title: 'Источник: '});
		/**
...
function deleteSelectedOperations(element){
	var gridId = UIButtons.findGrid(element);

	var column_id = 'checked_rows';
	var checked = [];
	...
	$('#' + gridId).find('table').children('tbody').children('tr').children('td').find('input[name="'+column_id+'"]').each(function (i) {
		if (this.checked) {
			checked.push($(this).closest('tr').find('td.authname').text());
		}
	});

	if( checked == '' ){
		var text = Yii.t('admin', 'Вы должны выбрать элементы перед удалением!');
