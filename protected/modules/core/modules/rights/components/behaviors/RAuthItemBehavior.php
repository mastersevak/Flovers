...
class RAuthItemBehavior extends CBehavior
{
	...
	*/
	...
	{
		return (Rights::module()->displayDescription===true && !empty($this->owner->description)) ? $this->owner->description : $this->owner->name;
	}
		...
		return CHtml::link(Rights::t('core', 'Revoke'), '#', array(
			'onclick'=>"
				jQuery.ajax({
					...
					data:{ ajax:1 $csrf },
					success:function() {
						$.fn.yiiGridView.update(UIButtons.findGrid());
					}
				});
