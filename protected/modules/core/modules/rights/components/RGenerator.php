...
class RGenerator extends CApplicationComponent
{
	...
	*/
	public function getControllerActions($items=null, $module_name = '')
	{
		if( $items===null )
		...
		foreach( $items['controllers'] as $controllerName=>$controller )
		{
			...
			while( feof($file)===false )
			{
				...
				if( $matches!==array() )
				{
					...
					//find external actions
					preg_match('/class.+[\.\']([a-zA-Z]+)Action\'/', $line, $matches);
					if( $matches!== array() ){
						$name = $matches[1];
						$actions[ strtolower($name) ] = array(
							'name'=>$name,
			...
			$result['controllers'][ $controllerName ]['actions'] = $actions;
			$result['controllers'][ $controllerName ]['path'] = $controller['path'];
		}

		foreach( $items['modules'] as $moduleName=>$module ){
			$result['modules'][ $moduleName ] = $this->getControllerActions($module, $moduleName);
		}
