...
class RDbAuthManager extends CDbAuthManager
{
	...
	*/
	...
	/**
	 * эту функция переопределена
	 */
	public function checkAccess($itemName,$userId,$params=array())
	{
		$assignments=$this->getAuthAssignments($userId);
		$result = $this->checkAccessRecursive($itemName,$userId,$params,$assignments);

		if(!$result && ($pos = strripos($itemName, ".")) !== false){ //смотрим еще вариант со звездочкой
			$itemName = str_replace(substr($itemName, $pos + 1), "*", $itemName);
	...
	*/
	public function getAuthItem($name, $allowCaching=true)
	{
		...
		if( $allowCaching && isset($this->_items[ $name ]) )
		{
			...
			//это добавил я для таких вариатов Admin.Back.*
			if(($pos = strripos($name, ".")) !== false){
				$name = str_replace(substr($name, $pos + 1), "*", $name);
